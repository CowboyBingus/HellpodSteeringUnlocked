...
Install `HellpodSteeringUnlocked.zip` with `BingusSharedLoader.zip` (loader-v2 / API 1). With the game closed, replace old library entries, Purge, then Deploy. Remove older gameplay packages that bundled their own loader. Managers do not fetch dependencies automatically.
