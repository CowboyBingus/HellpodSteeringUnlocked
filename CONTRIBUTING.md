...
The builder compiles Lua, runs the runtime checks in synthetic local allocations, inspects the archive and checks the final ZIP. It writes `releases/Hellpod-Steering-Unlocked-v7.4.zip`; intermediates, results and SHA256 are in `build/`. It never installs or launches the game.
