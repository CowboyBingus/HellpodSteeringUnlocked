...
The builder compiles Lua, runs the runtime checks in synthetic local allocations, inspects the archive and checks the final ZIP. It writes `releases/HellpodSteeringUnlocked.zip`; intermediates, results and SHA256 are in `build/`. It never installs or launches the game.
